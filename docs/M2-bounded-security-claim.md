...
| C4 | Record-number mask: PRF security + privacy | New construction §4.2.1, self-contained bound (derived in design-01 §4.2.1 / `mask-prf-proof.md` Thm 1/1′); sn_key is HKDF-PRF-independent from the AEAD key — same traffic secret, distinct label, `≤ Adv^{PRF}_{HMAC-Ascon-Hash256}` (design-01 §4.2.1) | Conservative `≤ q^2/2^192 + q/2^128`; tight single-block `≤ q/2^128` (Thm 1′ under `Hreducible`), negligible to `q ≈ 2^96` (conservative) / `q ≈ 2^128` (tight) — at wire cap `q=2^48`, `≤ 2^−80` |
