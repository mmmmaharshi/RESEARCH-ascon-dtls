...
| Cortex-M0 Ascon object size — 32-bit-word build | 9,476 bytes (ascon.o; 11,848 B full suite incl. cookie) |
| Cortex-M0 cycle test (32-bit-word build) | Run in Renode (Cortex-M0+ @32MHz): ASCON-AEAD128 0.409 MiB/s vs AES-128-GCM 0.071 MiB/s (~5.8× faster; Renode emulation estimate, not silicon). Size-opt (64-bit-word) build: 0.338 MiB/s. See `renode-benchmark-results.md`. |
| Cortex-M3 cycle test (32-bit-word build) | Run in Renode (Cortex-M3 @32MHz): ASCON-AEAD128 0.749 MiB/s vs AES-128-GCM 0.166 MiB/s (~4.5× faster; Renode emulation estimate, not silicon); ChaCha20-Poly1305 2.725 MiB/s (~16× faster than AES-GCM; Renode emulation estimate, not silicon). Size-opt (64-bit-word) build: 0.585 MiB/s. See `renode-benchmark-results.md`. |
| X.509 mode with peer verification enabled | PASS |
| Forced KeyUpdate on failed authentication (RFC 9846 §4.7.3) | PASS |
| Cortex-M0+/M3 per-record Ascon cost (32-bit-word build) | encrypt 3374/1862 cyc, decrypt 3459/1925 cyc, mask 1105/667 cyc (32-byte record, @32 MHz, Renode). Size-opt (64-bit-word) build: encrypt 4043/2341 cyc, decrypt 4128/2404 cyc, mask 1275/787 cyc. See `renode-benchmark-results.md`. |
| Build-configuration note (M2) | Two Ascon builds: **32-bit-word** (`WOLFSSL_ASCON_32BIT`, default) is faster but larger — used for all cycle/throughput figures unless marked "size-opt"; **size-optimized 64-bit-word** (`-UWOLFSSL_ASCON_32BIT`) is smaller but slower — yields the ≈1.25× footprint win. Never mix a number from one build with a claim scoped to the other; both are now measured in `renode-benchmark-results.md`. |
...
The PSK test used `dtls13_psk_server.exe` and `dtls13_psk_client.exe` — an **external PSK** (RFC 9147 §5.1), **`psk_ke`** primary mode (256-bit / 32-byte key, identity `Client_identity`); `psk_dhe_ke` was also exercised against OpenSSL 3.6.3 in **TLS 1.3 over TCP** (§`dtls13-ascon-validation.md` §11.1; cross-stack DTLS 1.3 interop is out of scope). The
